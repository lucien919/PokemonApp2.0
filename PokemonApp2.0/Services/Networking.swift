...
class Networking{
    ...
    class func callAPI(_ url:URL,_ type:PokemonListType, completion:@escaping(Any?, Error?)->()){
        ...
        session.dataTask(with: url){
            ...
            switch type{
            case .root:
                returnVal = parseRoot(data)
            case .type:
                returnVal = parseSharedType(data)
            case .ability:
                returnVal = parseSharedAbility(data)
            case .helditem:
                returnVal = parseSharedHeldItem(data)
            case .evolution:
                returnVal = parseEvolutionChain(data)
            case .detailpokemon:
                returnVal = parseDetailedPokemon(data)
            default:
                completion(nil, NetworkError.CannotParse)
            }
    ...
    private static func parseRoot(_ data:Data)->Any?{
        do{
    ...
    private static func parseSharedType(_ data:Data)->Any?{
        do{
            let json = try JSONSerialization.jsonObject(with: data)
    ...
    private static func parseSharedAbility(_ data:Data)->Any?{
        do{
            let json = try JSONSerialization.jsonObject(with: data)
    ...
    private static func parseSharedHeldItem(_ data:Data)->Any?{
        do{
            let json = try JSONSerialization.jsonObject(with: data)
    ...
    private static func parseEvolutionChain(_ data:Data)->Any?{
        do{
    ...
    private static func parseDetailedPokemon(_ data:Data)->Any?{
        do{
            let json = try JSONSerialization.jsonObject(with: data)
